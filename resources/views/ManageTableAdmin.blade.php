                    ...
                    <div class="table-list">
                        @foreach ($tables as $table)
                            <div class="table-status mb-3 selectable-table {{ $table->status == 1 ? 'open' : 'close' }}"
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    <div class="table-info">บิล
                                        @if ($table->bill->where('status', 0)->count() > 0)
                                            {{ $table->bill->where('status', 0)->last()->id }}
                                        @else
                                            -
                                        @endif
                                    </div>
                                    ...
                                    <div class="d-flex justify-content-end mt-3">
                                        @if ($selectedTable->status == 1)
                                            <button class="btn btn-success" data-bs-toggle="modal"
                                                ...
                                                onclick="openModal({{ $selectedTable->id }})">เปิดโต๊ะ</button>
                                        @else
                        ...
                        <form id="openTableForm" action="{{ route('bill.create') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group justify-content-center">
                                    ...
                                    <input type="text" class="form-control text-center" id="people-amount" name="amount"
                                        value="1" readonly>
                                    <button type="button" class="btn btn-outline-secondary"
                                        onclick="updatePeopleCount(1)">+</button>
                                </div>
                            ...
                            <div class="modal-footer justify-content-center">
                                <button type="submit" class="btn btn-success"
                                    onclick="startTimer()">เริ่มจับเวลา</button>
                            </div>
                        </form>
                    </div>
                </div>
                        ...
                        <form id="openTableForm" action="{{ route('bill.update') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group justify-content-center">
                                    <button type="button" class="btn btn-outline-secondary"
                                        onclick="updatePeopleCount(-1)">-</button>
                                    <input type="text" class="form-control text-center" id="people-amount"
                            ...
                            <div class="mb-3">
                                <label for="time-remaining" class="form-label">เวลา</label>
                                <input type="text" class="form-control text-center" id="time-remaining"
                                    name="time_remaining" readonly> <!-- แสดง countdown ที่เหลือ -->
                            </div>
